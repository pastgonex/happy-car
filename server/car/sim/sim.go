...
type PosSubscriber interface {
	Subscribe(context.Context) (ch chan *happyenvpb.CarPosUpdate, cleanUp func(), err error)
}

// Controller defines a car simulation controller.
type Controller struct {
	CarService    carpb.CarServiceClient
	AIService     happyenvpb.AIServiceClient
...
func (c *Controller) lockCar(ctx context.Context, car *carpb.CarEntity) (*carpb.CarEntity, error) {
	...
	_, err = c.AIService.EndSimulateCarPos(ctx, &happyenvpb.EndSimulateCarPosRequest{
		CarId: car.Id,
	})
	if err != nil {
...
func (c *Controller) unlockCar(ctx context.Context, car *carpb.CarEntity) (*carpb.CarEntity, error) {
	...
	_, err = c.AIService.SimulateCarPos(ctx, &happyenvpb.SimulateCarPosRequest{
		CarId: car.Id,
